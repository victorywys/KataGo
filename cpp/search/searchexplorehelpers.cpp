...
double Search::getFpuValueForChildrenAssumeVisited(
  ...
  if(searchParams.fpuParentWeightByVisitedPolicy) {
    ...
    parentUtilityForFPU = avgWeight * parentUtility + (1.0 - avgWeight) * getUtilityFromNN(*(node.getNNOutput()), boardWeights);
  }
  else if(searchParams.fpuParentWeight > 0.0) {
    // Use weighted version if weights are available
    parentUtilityForFPU = searchParams.fpuParentWeight * getUtilityFromNN(*(node.getNNOutput()), boardWeights) 
